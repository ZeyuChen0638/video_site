...
class PCDLoader extends Loader {
	...
	parse( data ) {
		...
		if ( PCDheader.data === 'binary_compressed' ) {
			...
			for ( let i = 0; i < PCDheader.points; i ++ ) {
				...
				if ( offset.x !== undefined ) {
					...
					position.push( dataview.getFloat64( ( PCDheader.points * offset.x ) + PCDheader.size[ xIndex ] * i, this.littleEndian ) );
					position.push( dataview.getFloat64( ( PCDheader.points * offset.y ) + PCDheader.size[ yIndex ] * i, this.littleEndian ) );
					position.push( dataview.getFloat64( ( PCDheader.points * offset.z ) + PCDheader.size[ zIndex ] * i, this.littleEndian ) );
					// position.push(12)
					console.log(position[i*3])
		...
		if ( position.length > 0 ) geometry.setAttribute( 'position', new Float64BufferAttribute( position, 3 ) );
